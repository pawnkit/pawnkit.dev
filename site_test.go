...
func TestBuildAndSearchIndex(t *testing.T) {
	...
	for _, entry := range entries {
		if entry.URL == "/reference/rule/missing-include.html" {
			foundRule = true
			if entry.Kind != "rule" || entry.Version != "v1.8.43" || entry.Repository == "" || entry.RawURL == "" {
	...
	assertContains(t, "dist/guides/compatibility-report.html", "toolchain-signed-2026-08-03-20.json")
	assertContains(t, "dist/guides/compatibility-report.html", "actions/runs/30807232388")

	if _, err := os.Stat(filepath.Join("dist", "raw", "rule", "v1.8.43", "missing-include.md")); err != nil {
		t.Fatal(err)
	}
	...
	releaseSet, err := os.ReadFile(filepath.Join("dist", "release-sets", "toolchain-signed-2026-08-03-20.json"))
	if err != nil {
		t.Fatal(err)
	}
