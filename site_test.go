...
func TestBuildAndSearchIndex(t *testing.T) {
	...
	for _, entry := range entries {
		if entry.URL == "/reference/rule/missing-include.html" {
			foundRule = true
			if entry.Kind != "rule" || entry.Version != "v1.7.39" || entry.Repository == "" || entry.RawURL == "" {
	...
	assertContains(t, "dist/support.html", "/raw/support/pawn-parser/v1.5.5/support.json")
	assertContains(t, "dist/search.html", `aria-live="polite"`)
	assertContains(t, "dist/reference/rule/missing-include.html", "Raw file")

	if _, err := os.Stat(filepath.Join("dist", "raw", "rule", "v1.7.39", "missing-include.md")); err != nil {
		t.Fatal(err)
	}
	...
	releaseSet, err := os.ReadFile(filepath.Join("dist", "release-sets", "toolchain-signed-2026-07-29-2.json"))
	if err != nil {
		t.Fatal(err)
	}
