...
func TestBuildAndSearchIndex(t *testing.T) {
	...
	for _, entry := range entries {
		if entry.URL == "/reference/rule/missing-include.html" {
			foundRule = true
			if entry.Kind != "rule" || entry.Version != "v1.8.34" || entry.Repository == "" || entry.RawURL == "" {
	...
	assertContains(t, "dist/support.html", "/raw/support/pawn-parser/v1.5.9/support.json")
	assertContains(t, "dist/search.html", `aria-live="polite"`)
	assertContains(t, "dist/reference/rule/missing-include.html", "Raw file")

	if _, err := os.Stat(filepath.Join("dist", "raw", "rule", "v1.8.34", "missing-include.md")); err != nil {
		t.Fatal(err)
	}
